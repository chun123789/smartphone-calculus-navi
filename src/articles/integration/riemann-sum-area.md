...
description: リーマン和→面積（定積分の意味）を図と式で結び、意味と使いどころを整理して見た瞬間に手順を再現できるようにする。
section: integration
order: 28
track: regular
intent: definition
...
cta: 次の基礎記事へ進む
hookQuestion: リーマン和→面積（定積分の意味）で、最初にどこを見れば迷わず進めるか？
oneLineAnswer: 定義を図と式で往復し、リーマン和→面積（定積分の意味）を自分の言葉で説明できる状態を目指す。
keyTakeaways:
  - 条件を言葉にし、変化する量と固定する量を分ける
  - 式変形の前にグラフで符号と範囲を確認する
  - 最後に符号・区間・単位を30秒で点検する
checkpointQuestions:
  - リーマン和→面積（定積分の意味）の要点を30秒で口頭説明できるか。
  - リーマン和→面積（定積分の意味）の同型問題で同じ手順を再現できるか。
tags:
  - 積分
  - 定期テスト
  - definition
